...
public class rotateObject : MonoBehaviour {
	...
	void Update () {
		...
		if(state == 0)
		{
			texto1.SetActiveRecursively(true);
		...
		}else if (state == 1)
		{
			...
			texto1.SetActiveRecursively(false);
			
		
			if(elapsedTime < 5 )
			{
				texto2.SetActiveRecursively(true);
				transform.RotateAroundLocal(Vector3.up, 5*Time.deltaTime*0.1f);
				transform.RotateAroundLocal(Vector3.forward, 5*Time.deltaTime*0.2f);
				transform.RotateAroundLocal(Vector3.right, 5*Time.deltaTime*0.3f);
			...
			{
				texto2.SetActiveRecursively(false);
				texto3.SetActiveRecursively(true);
